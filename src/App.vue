...
onMounted(async () => {
    ...
    window.ipcRenderer.receive('php.path.reply', (phpPath) => {
        ...
        if (!settingsStore.settings.php || phpPath.type === 'FIXED') {
            settingsStore.settings.php = phpPath.path
            settingsStore.update()
            if (phpPath.type === 'FIXED') {
                alert("Fixed PHP path successfully")
            }
        }
    ...
    setTimeout(() => {
        window.ipcRenderer.send('php.path', { type: 'INITIAL' })
    }, 1000)
})
...
<template>
    <div class="h-full" :style="{ color: settingsStore.colors.foreground }">
        <TitleBar />
        <aside class="w-12 fixed z-40 top-[38px] left-0 bottom-0 justify-between border-r" :style="{
